...
import com.yc.phonogram.utils.CommonUtils;
...
public class MainActivity extends BaseActivity implements EasyPermissions.PermissionCallbacks {
    ...
    public void computeTime() {
        if (timeNum > 12 && !App.isTrial) {
            SPUtils.getInstance().put(Config.FOLLOW_DATE, TimeUtils.getNowString());
            LogUtils.i("关注有效开始时间--->" + TimeUtils.getNowMills() + "----" + TimeUtils.getNowString());
            timeNum = 0;//保存时间后就恢复为0
        }

        String followTime = SPUtils.getInstance().getString(Config.FOLLOW_DATE, "");
        //判断是否符合试用要求
        CommonUtils.initTrial(followTime);

        LogUtils.i("isTrial--->" + App.isTrial);
    }
